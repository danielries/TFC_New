...
def data_generator(sourcedata_path, targetdata_path, configs, training_mode, subset = True):
    ...
    train_loss = torch.load(os.path.join(sourcedata_path, "test.pt"))
    ...
    train_loss = Load_Dataset(train_loss, configs, training_mode, target_dataset_size=configs.batch_size, subset=subset)
    ...
    loss_loader = torch.utils.data.DataLoader(dataset=train_loss, batch_size=configs.batch_size,
                                                  shuffle=True, drop_last=configs.drop_last,
                                                    num_workers=0)
